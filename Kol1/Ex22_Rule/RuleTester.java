...
class RuleProcessor
{
    ...
    {
        inputs.stream().forEach(currInput ->
        {
            System.out.println("Input: "+ currInput);

            rules.forEach(rule ->
            {
                Optional<OUT> result = rule.apply(currInput);
